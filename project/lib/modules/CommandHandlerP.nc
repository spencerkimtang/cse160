...
implementation{
    task void processCommand(){
        if(! call Queue.empty()){
            ...
            //for project 4
            case CMD_SETAPPSERVER:
                dbg(COMMAND_CHANNEL,"Command Type: setting app server");
                signal CommandHandler.setAppServer();
                break;
            
            case CMD_SETTINGAPPCLIENT:
                dbg(COMMAND_CHANNEL,"Command Type: setting app client");
                signal CommandHandler.settingAppClient(buff[0], &buff[1]);
                break;

            case CMD_CONNECTED:
                dbg(COMMAND_CHANNEL,"Command Type: server is connected");
                signal CommandHandler.connectServer(buff[0], buff[1], buff[2], &buff[3]);
                break;

            case CMD_BROADCAST:
                dbg(COMMAND_CHANNEL,"Command Type: broadcasting message");
                signal CommandHandler.broadcast(buff[0], buff[1]);
                break;

            case CMD_UNICAST:
                dbg(COMMAND_CHANNEL,"Command Type: unicasting");
                signal CommandHandler.unicast(buff[0], buff[1], buff[2]);
                break;

            case CMD_PRINTUSERS:
                dbg(COMMAND_CHANNEL,"Command Type: printing users");
                signal CommandHandler.printUser(buff[0]);
                break;
