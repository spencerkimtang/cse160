...
class TestSim:
    ...
    def testClient(self, source, srcPort, destination, destPort, num_bytes_to_transfer):
        self.sendCMD(self.CMD_TEST_SERVER, source, "{0}{1}{2}{3}".format(chr(srcPort), chr(destination), chr(destPort), chr(num_bytes_to_transfer)));
    ...
    # project 4
    def setAppServer(self, source):
        self.sendCMD(self.CMD_setAppServer, source, "set app server");

    def setAppClient(self, source, port, username):
        self.sendCMD(self.CMD_settingAppClient, source, "{0}{1}".format(chr(port), username));

    def connected(self, source, server_addr, server_port, client_port, username):
        self.sendCMD(self.CMD_CONNECTED, "{0}{1}{2}{3}".format(chr(server_addr), chr(server_port), chr(client_port), username));

    def broadcast(self, source, client_port, message):
        self.sendCMD(self.CMD_BROADCAST, "{0}{1}".format(chr(client_port), message));

    def unicast(self, source, srcPort, destPort, message):
        self.sendCMD(self.CMD_UNICAST, "{0}{1}{2}".format(chr(srcPort), chr(destPort), message));

    def printUsers(self, soure, client_port):
        self.sendCMD(self.CMD_PRINTUSERS, "{0}".format(chr(client_port)));
...
def main():
    ...
    s.setAppServer(1);
    s.runtime(100);
    s.setAppClient(2, 34, "stang32");
    s.runtime(100);
    s.connected(2, 1, 41, 34);
    s.runTume(100);
    s.broadcast(2, 1, 41, 34, "hello world\r\n");
